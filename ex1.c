...
void copy_file(const char *source_file, const char *dest_file, int buffer_size, int force_flag) {
	 ...
	 */
     ...
     if(buffer == NULL){
      ...
      Fs = open(source_file, O_RDONLY);

     if(Fs == -1){
     	perror("Problem with open source file");
     	close(Fs);
     	exit(EXIT_FAILURE);
     }
     ...
     if(force_flag == 0){
     	...
     	Fd = open(dest_file, O_WRONLY | O_CREAT | O_EXCL , DESTINATION_FILE_MODE);

     	if(Fd < 0){
           perror("Problem with open destination file");
           close(Fs);
           exit(EXIT_FAILURE);
     	}
     }
     	...
     	Fd = open(dest_file, O_CREAT| O_TRUNC |O_WRONLY , DESTINATION_FILE_MODE);
     ...
     if(Fd == -1){
     	perror("Problem with open destination file for writing");
     	close(Fs);
     	exit(EXIT_FAILURE);
     }

     Rline = read(Fs, buffer, buffer_size);

     if(Rline == -1){
     	perror("Error while reading from source_file");
     	close(Fs);
     	close(Fd);
     	exit(EXIT_FAILURE);
     }
     ...
     while(Rline != 0){
         ...
         Wline = write(Fd, buffer, buffer_size);

          if(Wline == -1){
     	    perror("Error while writing to dest_file");
     	    close(Fs);
          	close(Fd);
     	    exit(EXIT_FAILURE);
          }
	 ...
	 printf("File %s  was copied to %s", source_file, dest_file);

	 //free Malloc
	 free(buffer);

	 if (close(Fs) == -1 || close(Fd) == -1) {
